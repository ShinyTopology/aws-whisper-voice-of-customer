...
```bash
voc-stepfunctions$ sam build --use-container
```
...
```bash
voc-stepfunctions$ sam logs -n StockCheckerFunction --stack-name "voc-stepfunctions" --tail
```
...
```bash
voc-stepfunctions$ pip install -r tests/requirements.txt --user
# unit test
voc-stepfunctions$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
voc-stepfunctions$ AWS_SAM_STACK_NAME="voc-stepfunctions" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "voc-stepfunctions"
```
